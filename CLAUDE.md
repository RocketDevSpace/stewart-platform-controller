...
control/
  ...
  autotune.py          # PDAutotuner — SysID pipeline: probe → fit → design → suggest (background compute thread). ✅
  plant_model.py       # ball-on-plate physics: PlantParams, plant_step, fractional-latency replay, vectorized batch replay. The ONE plant model. ✅
  plant_id.py          # system ID: ProbeScript (~78 s, gate-free), ProbeRecording, convex latency-scan regression fit + confidence. ✅
  gain_design.py       # offline kp/ki/kd search on the fitted plant: CRN-seeded sim objective, current gains always in the pool. ✅
  ...
  orbit.py             # HarmonicOrbit — feedforward-driven smooth circles: clock-driven reference + analytic rotating tilt + learned per-phase ILC table; entrain/track/recover. Injected clock, no Qt. ✅

cv/
  camera_source.py     # camera lifecycle: backend probe, capture thread, exposure policy, software gain, frame callback. ✅
  ball_tracker.py      # pure detection: homography source ladder (quad → aruco → stale → miss) + HSV blob → BallState. No camera, no threads. ✅
  quad_tracker.py      # PlatformQuadTracker — boundary-edge platform tracking (PRIMARY homography source; ArUco = seed/audit/fallback). Pure numpy+cv2. ✅
  measurement_filter.py  # pure position/velocity filtering (modes: adaptive alpha_beta / legacy / raw) + PointDeadbandLP (shared marker/quad-corner LP); bench-importable. ✅
...
tests/                 # test_safety, test_serial_manager, test_servo_driver, test_ik_engine, test_ik_solver, test_routine_runner, test_ball_controller (+ characterization, + paths, + orbit), test_ball_tracker, test_quad_tracker, test_orbit, test_orbit_sim, test_camera_source, test_vision_control_worker, test_settings_store, test_settings_overlay, test_measurement_filter, test_rest_gate, test_jitter_bench, test_patterns, test_path_follower, test_path_feasibility, test_pid_core_integral, test_plant_model, test_plant_id, test_gain_design.
```
...
**2026-07-23 SysID autotune rework** (branch `rework/autotune-id`, stacked on the I-term rework): the step-test autotune estimator was deleted after being convicted on evidence — zero legs ever completed on the rig (settle gates never opened against the wobble floor), and it random-walked when run against a KNOWN simulated plant (its 2-scalar inversion assumes an ideal 2nd-order plant; the real one has 2-3 frames latency, stiction, warp, and the ball's self-rock). Replacement: **measure the plant once, design in software** — `PDAutotuner` now runs a ~78 s gate-free scripted probe (relay toggles + stiction micro-steps + baseline steps) recording raw detections + sent commands; fits (g_eff, latency, stiction, warp, biases) by a convex per-latency acceleration regression (`plant_id.py` — raw positions deliberately, so the filter's group delay is measured separately by cross-correlation instead of contaminating the plant latency; the self-rock band is notched from both sides to kill closed-loop ID bias; under strong rock the reported stiction is the dither-linearized effective value); then searches kp/ki/kd on the fitted plant with CRN-paired sims (`gain_design.py` — the current gains are always in the pool, so a suggestion can never be measured-worse than today). Apply lands all three gains AND calibrates `_predict_s` (fitted pipeline latency + measured filter lag) and the integral deadband, persisting everything to the overlay (16-key whitelist now). Compute runs on an autotuner-owned daemon thread; `run_compute_inline` for tests. The end-to-end killer test runs a whole session closed-loop against a known plant and requires parameter recovery + a never-worse suggestion. Post-apply on-rig validation re-probe: deferred follow-up.

**2026-07-27 harmonic orbit** (same branch): a SEPARATE smooth-circle mode (path following preserved): `control/orbit.py` drives a CLOCK-paced circular reference (no error-paced carrot — that coupling is the circle jank source) with analytic feedforward (rotating centripetal tilt, phase-advanced by the actuation delay) plus a learned per-phase ILC correction table that absorbs the plate-specific warp/drag harmonics (bigger than the analytic term: 0.28° warp vs 0.19° centripetal at r=50/v=40). Feedback is demoted to trim (`PIDCore.compute(gain_scale=0.5)`, p/d only); the integral owns DC during entrain/recover and freezes through track (running, it out-gains the scaled P at orbit frequency with 90° lag and fights the table — sim-caught, as were the bin-transit + Gaussian-kernel write rules that keep learning below the scaled-gain resonance). Measured sim A/B on the shared rig-warp + servo-lag plant: orbit converges to 1.9 mm mean radius error / 0.8 mm ripple vs the carrot's 7.0 mm radius error; speed std 0.38 vs 0.55 mm/s; kick-recovery pinned. GUI: Harmonic Orbit toggle + radius spinbox (speed shares the Path Speed slider), mutual exclusion with path/autotune/home-cal everywhere.

**2026-07-24 boundary-quad platform tracking** (same branch `rework/autotune-id`): rig data proved the ball occludes the ArUco markers during path transits (>4 mm single-frame homography jumps on 19% of frames near the marker diagonals, 3× baseline — the markers sit at ±60 mm, in the ball's traffic), so the homography source is now the platform's BOUNDARY (±120 mm, physically unreachable by the ball): `cv/quad_tracker.py` fits the four gray boundary edges (32 sub-pixel samples/side via one `cv2.remap`, trimmed TLS lines, corner intersection, deadband corner LP → bit-stable H at rest) and `BallTracker` resolves H through the ladder quad → aruco → stale-hold → miss (`homography_source` per frame; `TRACKER_QUAD_ENABLED=False` restores the pre-quad path). ArUco is demoted to acquisition seed (identity/scale + per-side polarity/offset calibration), every-15th-frame audit (skipped while the ball is within 45 mm of a marker — a slow transit otherwise reads as persistent disagreement and reacquires from the glitched H; sim-caught), and fallback (gray-on-gray fails CLOSED). Deterministic A/B pin: the ArUco baseline reproduces the rig glitch on the synthetic transit sweep (max 7.7 mm), the quad path shows zero >4 mm jumps (max 1.2 mm). GUI: fitted-quad overlay + `H: QUAD/ARUCO/STALE/--` badge on the camera view, `quad_fit` timing series, `[TRACK]` transition log. Rig gate: cover-a-marker test keeps the badge QUAD; the angle-folded glitch rate near the diagonals must drop from 19% toward the ~6% baseline.
