...
class StewartGUIView(QWidget):
    ...
    def _build_ui(self):
        ...
        camera_panel = self._build_camera_panel()
        comms_panel = self._build_comms_panel()

        root.addWidget(control_panel, 0, 0)
        root.addWidget(telemetry_panel, 0, 1)
        root.addWidget(camera_panel, 1, 0, 1, 2)
        root.addWidget(comms_panel, 0, 2, 2, 1)
    ...
    def _build_telemetry_panel(self):
        box = QGroupBox("Visualizer + Telemetry")
        layout = QVBoxLayout()
        ...
        self.timing_figure = Figure(figsize=(6, 2.2))
    ...
    def _build_camera_panel(self):
        ...
        for lbl in [self.camera_view_label, self.warped_view_label, self.mask_view_label]:
